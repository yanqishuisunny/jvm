 ...
 * 所谓编译执行，就是通过即时编译器(Just In Time, JIT)将字节码转换为本地机器码来执行；现代JVM
 * 会根据代码热点来生成相应的本地机器码
 *
 * 基于栈的指令集与基于寄存器的指令集之间的关系
 * 1: JVM执行指令时所采取的方式是基于栈的指令集
 * 2：基于栈的指令集主要的操作有入栈和出栈俩种
 * 3:基于栈的指令集的优势在于它可以在不同平台之间移植，而基于寄存器的指令集是与硬件架构紧密关联的，无法做到可移植。
 * 4:基于栈的指令集的缺点在于完成相同的操作，指令数量通常要比基于寄存器的指令集数量要多；基于栈的指令集是在内存中
 *   完成操作的，而基于寄存器的指令集是直接由CPU来执行的，他是在高速缓冲区中进行执行的，速度要快很多。虽然虚拟机
 *   可以采用一些优化手段。但总体来说，基于栈的指令集的执行速度要慢一些。
 */
